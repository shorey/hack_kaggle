...
from supervised_learning import PolynomialRidgeRegression
from utils import k_fold_cross_validation_sets, normalize, Plot
from utils import train_test_split, polynomial_features, mean_squared_error
...
def main():
    ...
    #find better regularization coef using cross validation
    lowest_error = float("inf")

    best_reg_factor = None
    print("Finding regulariztion coef using cross validation:")
    k = 10
    for reg_factor in np.arange(0,0.1,0.01):
        ...
        # save reg coef that gives lowest error
        if mse < lowest_error:
