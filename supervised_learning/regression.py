...
class Regression():
    """
    base regression model. models the relationship bwtween a scalar dependent variable y and the independent variables x.
...
class LassoRegression(Regression):
    ...
    and regularization.
    model that tries to balance the fit of the model with respect to the training data and the
...
class PolynomialRegression(Regression):
    """
    perform a non-linear transformation of the data before fitting the model and doing predictions
    which allows for doing non-linear regression.

    parameters:
    -----------
    degree: int
        the degree of the polynomial that the independetn variable X will be transformed to.
    n_iterations: float
        the number of training iterations the algorithm will tune the weights for.
    learning_rate: float
        the step lenght that will be used when updating the weights.
    """
